...
const atxIndex = $ => {
    return $.findIndex(v => v === $[512]);
}

const weatherStates = $ => {
    const out = [];
    $.forEach(v => out.push(v.weather_state_name));
    return out;
}

const idealTemp = ($, min) => {
    return $.find(v => v.min_temp === min).id;
}

c(stringsToNums(pi));
c(sumEvens(pi));
c(`Index: ${atxIndex(pi)}, Value: ${pi[atxIndex(pi)]}`);
c(weatherStates(weather));
c(idealTemp(weather, 15.915));
